...
orr(.data = df_avg, .x = "vo2_abs", .y = "vco2", vo2 = "vo2_abs")
...
loop.orr <- function(df, .x, .y){
    ss_both <- vector(length = nrow(df)-3)
    for(i in 2:(nrow(df)-2)) {
        # split data into left and right halves
        # should these share the same point? Or should, they be different by 1 point?
        df_left <- .data[1:i,]
        df_right <- .data[(i+1):nrow(df),] # currently set up to differ by 1 point
