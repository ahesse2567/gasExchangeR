...
v_slope <- function(.data,
  ...
  while (slope_change < slope_change_lim) {
    ...
    if (i > length(dist_MSE_ratio_idx[!is.na(dist_MSE_ratio_idx)])) {
      message(paste0(
        "No VT1 breakpoint found with v-slope method because the change between slopes was never >= ",
        slope_change_lim, "."
      ))
      bp_dat <- .data %>%
        dplyr::slice(1) %>%
...
loop_v_slope <- function(.data,
  ...
  for (i in 1:n_rows) {
    ...
    pf_two[i] <- stats::pf(
      f_stat[i],
      df1 = 2, df2 = nrow(lm_simple$model) - 4,
      lower.tail = FALSE
    )
