...
dmax <- function(.data,
  ...
  bp_dat <- find_threshold_vals(
    .data = .data, thr_x = D.max, thr_y = g.D.max,
    .x = .x, .y = .y
  )
  bp_dat <- bp_dat %>%
    dplyr::mutate(
...
my.distance.derivative <- function(f, g, FF) {
  ...
  D.deriv <- 2 * poly.product(
    poly.sum(FF, c(0, -1)),
    poly.sum(FF.deriv, c(-1))
  ) +
    2 * poly.product(
      poly.sum(poly.composition(f, FF), -g),
...
ComputeRoot <- function(p, x1, x2) {
  ...
  root.index <- intersect(
    which(roots.real > x1),
    which(roots.real < x2)
  ) # Get roots within interval
